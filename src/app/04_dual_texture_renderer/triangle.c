...
static void create_fbo_and_tex(int idx, struct screen_context *screen_ctx) {
    int w = (idx < 2 ? screen_ctx->screen_width / 2 : screen_ctx->screen_width);
    int h = screen_ctx->screen_height;
...
static GLuint fullscreen_quad_vbo = 0; // Static VBO for fullscreen quad
...
int init_triangle(struct screen_context *screen_ctx) {
  ...
  create_fbo_and_tex(0, screen_ctx);
  create_fbo_and_tex(1, screen_ctx);
  create_fbo_and_tex(2, screen_ctx);

  // Allocate fullscreen quad VBO
  static const float quad_uvs[] = {
    0.f, 0.f,  1.f, 0.f,  0.f, 1.f,  1.f, 1.f
  };
  glGenBuffers(1, &fullscreen_quad_vbo);
  glBindBuffer(GL_ARRAY_BUFFER, fullscreen_quad_vbo);
...
void draw_triangle(struct screen_context *screen_ctx) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        glViewport(0, 0, screen_ctx->screen_width / 2, screen_ctx->screen_height);
    ...
    glUniform1i(tex_loc, 0);
    glViewport(screen_ctx->screen_width / 2, 0, screen_ctx->screen_width / 2, screen_ctx->screen_height);
    draw_fullscreen_quad(display_program);
